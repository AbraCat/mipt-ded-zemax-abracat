...
hui::Widget::Widget(hui::State *state) : hui::Widget(300, 300, state, nullptr)
{
    // assert(0);
}

hui::Widget::Widget(const std::size_t width, const std::size_t height, 
    State *state, Widget *parent)
    : state(state), parent(parent), texture(getTexture(width, height))
{
    //
}
